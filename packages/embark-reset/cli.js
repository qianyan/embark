...
function spec(program) {
  ...
  program.description(
    [
      "resets state of DApp project directory",
      "  [paths...] must be relative to the current working directory and within the project directory"
    ].join("\n\n")
  );
  program.usage("[options] [paths...]");
  program.option(
    "-l --list",
    "list all paths that would be removed but do not remove them"
  );
  program.option("-d --list-defaults", "list only default removal paths");
  program.option(
    "-a --absolute",
    "when listing show aboslute paths instead of paths relative to current working directory"
  );
  program.option(
    "-C --no-config",
    "do not load command options and default paths from embark.json"
  );
  program.option(
    "-D --no-default-paths",
    "do not include default removal paths with paths supplied from cli and/or embark.json"
  );
