...
// const {spec: initSpec} = require('embark-init');
...
class Cmd {
  ...
  newApp() {
    ...
    program
      ...
      .option(
        "--template <name/url>",
        __("download a template using a known name or a git host URL")
      )
      .action(function(name, options) {
        ...
        if (name === undefined) {
          ...
          return promptly.prompt(
            __("Name your app (default is %s):", "embarkDapp"),
            {
              default: "embarkDApp",
  ...
  demo() {
    program
      ...
      .option(
        "--template <name/url>",
        __("download a demo template using a known name or a git host URL")
      )
      ...
      .action(function(options) {
        ...
        if (options.template) {
          ...
          const url = !hgi
            ? `embark-framework/embark-${options.template}-template#demo`
            : options.template;
          const folderName = !hgi
            ? `embark_${options.template}_demo`
            : "template_demo";
  ...
  build() {
    program
      ...
      .option(
        "--logfile [logfile]",
        __("filename to output logs (default: none)")
      )
      .option(
        "-c, --client [client]",
        __(
          "Use a specific ethereum client [%s] (default: %s)",
          "geth, parity",
          "geth"
        )
      )
      .option(
        "--loglevel [loglevel]",
        __("level of logging to display") +
      ...
      .option(
        "--pipeline [pipeline]",
        __("webpack config to use (default: production)")
      )
      .description(
        __("deploy and build dapp at ") + "dist/ (default: development)"
      )
  ...
  run() {
    program
      ...
      .option(
        "-p, --port [port]",
        __("port to run the dev webserver (default: %s)", "8000")
      )
      .option(
        "-c, --client [client]",
        __(
      ...
      .option(
        "-b, --host [host]",
        __("host to run the dev webserver (default: %s)", "localhost")
      )
      ...
      .option(
        "--no-color",
        __("no colors in case it's needed for compatbility purposes")
      )
      .option(
        "--logfile [logfile]",
        __("filename to output logs (default: %s)", "none")
      )
      .option(
        "--loglevel [loglevel]",
        __("level of logging to display") +
      ...
      .option(
        "--pipeline [pipeline]",
        __("webpack config to use (default: development)")
      )
      .option(
        "--no-single-use-auth-token",
        __("disable the single use of token in cockpit")
      )
  ...
  simulator() {
    program
      ...
      .option(
        "--testrpc",
        __(
          'use ganache-cli (former "testrpc") as the rpc simulator [%s]',
          "default"
        )
      )
      .option(
        "-p, --port [port]",
        __("port to run the rpc simulator (default: %s)", "8545")
      )
      .option(
        "--host [host]",
        __("host to run the rpc simulator (default: %s)", "localhost")
      )
      .option(
        "-a, --accounts [numAccounts]",
        __("number of accounts (default: %s)", "10")
      )
      .option(
        "-e, --defaultBalanceEther [balance]",
        __("Amount of ether to assign each test account (default: %s)", "100")
      )
      .option(
        "-l, --gasLimit [gasLimit]",
        __("custom gas limit (default: %s)", "8000000")
      )
      .option("--locale [locale]", __("language to use (default: en)"))
  ...
  test() {
    program
      ...
      .option(
        "-e, --env <env>",
        __("configuration environment to use (default: development)")
      )
      .option(
        "-n , --node <node>",
        __(
          'node for running the tests ["vm", "embark", <endpoint>] (default: vm)\n'
        ) +
          "                       vm - " +
          __("start and use an Ethereum simulator (ganache)") +
          "\n" +
          "                       embark - " +
          __("use the node of a running embark process") +
          "\n" +
          "                       <endpoint> - " +
          __("connect to and use the specified node")
      )
      .option(
        "--gasDetails",
        __(
          "print the gas cost for each contract deployment when running the tests. Deprecated: Please use --gas-details"
        )
      )
      .option(
        "-d , --gas-details",
      ...
      .option(
        "-t , --tx-details",
        __("print the details of the transactions that happen during tests")
      )
      .option(
        "-c , --coverage",
        __("generate a coverage report after running the tests (vm only)")
      )
      .option("--locale [locale]", __("language to use (default: en)"))
      .option(
        "--loglevel [loglevel]",
      ...
      .action(function(file, options) {
        ...
        if (!urlRegexExp.test(node)) {
          console.error(
            `invalid --node option: must be "vm", "embark" or a valid URL\n`.red
          );
          options.outputHelp();
          process.exit(1);
        }
        ...
        if (options.coverage && options.node !== "vm") {
          console.error(
            `invalid --node option: coverage supports "vm" only\n`.red
          );
          options.outputHelp();
          process.exit(1);
        }
    ...
    program
      ...
      .action(function(env, _options) {
        ...
        if (env === "ipfs" || env === "swarm") {
          console.warn(
            ("did you mean " + "embark upload".bold + " ?").underline
          );
          console.warn(
            "In embark 3.1 forwards, the correct command is embark upload <environment> and the provider is configured in config/storage.js"
          );
        }
        _options.env = env || "development";
  ...
  graph() {
    program
      ...
      .option(
        "--skip-undeployed",
        __("Graph will not include undeployed contracts")
      )
      ...
      .option(
        "--output [svgfile]",
        __("filepath to output SVG graph to (default: %s)", fs.diagramPath())
      )
      .description(
        __("generates documentation based on the smart contracts configured")
      )
      .action(function(env, options) {
        i18n.setOrDetectLocale(options.locale);
        embark.graph({
  ...
  ejectWebpack() {
    program
      ...
      .alias("eject-webpack")
      .description(
        __("copy the default build config into your dapp for customization")
      )
      .action(function() {
        embark.initConfig("development", {
          embarkConfig: "embark.json",
  ...
  helpCmd() {
    program
      ...
      .action(function() {
        console.log(
          "Documentation can be found at: ".green +
            "https://embark.status.im/docs/".underline.green
        );
        console.log("");
        console.log(
          "Have an issue? submit it here: ".green +
            "https://github.com/embark-framework/embark/issues/new".underline
              .green
        );
        console.log(
          "or chat with us directly at: ".green +
            "https://gitter.im/embark-framework/Lobby".underline.green
        );
        program.help();
        process.exit(0);
      });
